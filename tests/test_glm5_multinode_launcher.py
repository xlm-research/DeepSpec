...
class Glm5MultiNodeLauncherTest(unittest.TestCase):
    ...
    def test_deepep_options_only_change_draft_parallel_settings(self):
        result = self._run_launcher(
            TARGET_BACKEND="vllm",
            PARTITIONED_MODEL_SWAP="true",
            SAVE_CHECKPOINTS="true",
            ...
            DRAFT_EP_MAX_TOKENS=2048,
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        for expected in (
            "train.parallel.expert_dispatch_backend=deepep",
            "train.parallel.expert_dispatch_max_tokens_per_rank=2048",
            "train.parallel.ep=8",
            "train.parallel.dp_replicate=2",
            "train.partitioned_model_swap.target_backend=vllm",
            "train.partitioned_model_swap.vllm.python_executable=/bin/echo",
        ):
            self.assertIn(expected, result.stdout)
        self.assertNotIn("train.target_parallel.expert_dispatch_backend=deepep", result.stdout)

    def test_deepep_launch_rejects_invalid_capacity_backend_and_ep(self):
        common = dict(PYTHON_BIN="/bin/echo", CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7")
        for overrides, message in (
            ({"DRAFT_EP_BACKEND": "typo"}, "DRAFT_EP_BACKEND"),
            ({"DRAFT_EP_MAX_TOKENS": 0}, "DRAFT_EP_MAX_TOKENS"),
            ({"DRAFT_EP_BACKEND": "deepep", "DRAFT_EP": 1}, "DRAFT_EP"),
        ):
            with self.subTest(overrides=overrides):
                result = self._run_launcher(**common, **overrides)
                self.assertNotEqual(result.returncode, 0)
                self.assertIn(message, result.stderr)

    def test_deepep_launch_requires_explicit_existing_vllm_interpreter(self):
        result = self._run_launcher(
            TARGET_BACKEND="vllm", PARTITIONED_MODEL_SWAP="true",
            SAVE_CHECKPOINTS="true", PYTHON_BIN="/bin/echo",
            CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7", DRAFT_EP_BACKEND="deepep",
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Set VLLM_PYTHON_BIN", result.stderr)
    ...
    def test_sensecore_job_needs_no_manual_topology_variables(self):
        ...
        self.assertIn(
            "spec_o3_coldstartsft.repeat60.deepspec.jsonl",
            result.stdout,
        )
