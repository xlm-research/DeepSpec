...
def test_two_feature_partitions_preserve_inflight_gradients(tmp_path):
    ...
    trainer.parallel = SimpleNamespace(
        context_parallel_rank=0, data_parallel_rank=0, data_parallel_size=1
    )
    trainer.parallel_config = SimpleNamespace(cp=1)
    ...
    trainer.train_dataset = batches
    trainer.data_collator = lambda records: records[0]
...
def test_worker_releases_descendants_after_success_failure_or_timeout(
    ...
    # The success/error cases test descendant cleanup, not Python startup speed
    # on the shared filesystem. Give them enough time to reach their exit code.
    config = QwenVllmConfig(
        python_executable=str(executable),
        timeout_seconds=10 if exit_code is None else 30,
    )
