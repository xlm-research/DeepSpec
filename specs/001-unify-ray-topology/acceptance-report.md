...
## 2026-09-21 B300 单机 4K 补充验证

在 `dev-951e3d4b-0` 的 8 张 B300 上，使用 `env.sh` 激活共享 conda 环境，
四卡 vLLM TP4 + 四卡原生 TorchTitan TP4 已完成三步真实训练：12 条实际 4K
样本、48 次校验读取、四 rank 各三次更新和 checkpoint 提交。主流程与结束后
独立 CPU 核验均通过，732 个 checkpoint 字段完整、40 个参数变化，12 条源对象
及 15 项本轮资源全部释放。证据见 [4K 结果](../../outputs/b300_20260920_final/4k-result.json)
和 [核验报告](../../outputs/b300_20260920_final/4k/verification.json)。

用户将本次范围缩小为仅验证 4K，故未启动最终 128K 重跑；此前 128K 的训练及
checkpoint 核验虽完成，但一次 GPU 资源采样超时使最终指标核验失败，保留为失败
记录。单机结果不替代 M1–M3 多节点、旧/新入口等价对照或完整 SC-001–010 验收。
修复说明、环境证据与后续多机命令见 [B300 调试交接](../../scripts/train/qwen3.8_ray_mooncake_vllm_torchtitan/B300_DEBUG.md)。

## 此前完整矩阵验收快照

以下保留 B300 补充验证前的记录；其中 M0-4K 当时的阻塞已由上述新证据补充。
