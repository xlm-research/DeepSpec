...
def hook(name,module,args,output):
    ...
    if difference.count_nonzero():
        ...
        torch.save({'input':value.cpu(),'weight':weight.cpu(),'reference':output.detach().cpu()},root/(name+'-contiguous-rounding.pt'))
for name,module in model.named_modules():
    if name.startswith('layers.') and (isinstance(module,torch.nn.Linear) or name.endswith(('q_norm','k_norm'))):
...
for name,(value,weight,grad) in records.items():
    if weight.ndim == 1:
        def rms(value,weight):
            ...
            normalized=value.float()*torch.rsqrt(variance+1e-6)
            return weight*normalized.to(value.dtype)
        with torch.enable_grad():
            whole=value.detach().requires_grad_()
            scale=weight.detach().requires_grad_()
            expected,expected_scale=torch.autograd.grad(rms(whole,scale),(whole,scale),grad)
            parts=[]; scales=[]
            for x,g in zip(value.chunk(4,-2),grad.chunk(4,-2)):
                x=x.contiguous().detach().requires_grad_()
                scale=weight.detach().requires_grad_()
                dx,dw=torch.autograd.grad(rms(x,scale),(x,scale),g.contiguous())
                parts.append(dx); scales.append(dw)
            actual=torch.cat(parts,-2)
        d=(actual-expected).abs()
        print(name,'norm_input_gradient','different',d.count_nonzero().item(),'max',d.max().item(),flush=True)
        continue
    ...
    if name.rsplit('.',1)[-1] in ('o_proj','down_proj'):
        actual_input=torch.cat([grad.float() @ w.contiguous().float() for w in weight.chunk(4,1)],-1).to(value.dtype)
        actual_weight=torch.cat([grad.float().T @ x.float() for x in value.chunk(4,1)],1).to(value.dtype)
    else:
        partials=[g.contiguous().float() @ w.contiguous().float() for g,w in zip(grad.chunk(4,-1),weight.chunk(4,0))]
        actual_input=(partials[0]+partials[1]+partials[2]+partials[3]).to(value.dtype)
        actual_weight=torch.cat([g.contiguous().T @ value for g in grad.chunk(4,-1)],0)
    complete_input=torch.cat([grad @ w.contiguous() for w in weight.chunk(4,1)],-1)
    ...
    for label,actual,expected in [('input_gradient',actual_input,expected_input),('weight_gradient',actual_weight,expected_weight)]:
        ...
        if d.count_nonzero():
            ...
            torch.save({'input':value.cpu(),'weight':weight.cpu(),'grad':grad.cpu(),'reference_input_gradient':expected_input.cpu(),'reference_weight_gradient':expected_weight.cpu()},root/(name+'-contiguous-backward-rounding.pt'))
