...
- [ ] 通过原生 AC 配置对真实 DSpark blocks 应用 SelectiveAC；每个 block 采用一套 checkpoint 策略，不叠加 HF 与 TorchTitan checkpoint 包装。
- [ ] 先关闭外层 model compile，在两卡 FSDP2 下比较 AC 关闭与开启，覆盖 FP32/BF16、GAS ≥ 2、不等有效分母和至少两个 updates。
- [ ] 比较 loss、全部可训练梯度、clip norm、master/Adam/scheduler 和重计算 RNG；保留票 01 的匹配训练语义与容差。
- [ ] 从同一 DeepSpec 阶段入口完成 DCP、全部 worker 退出、新进程同拓扑恢复及下一 update；重建后 AC 配置和随机序列保持一致。
- [ ] 记录实际显存和重计算成本，给出合法配置与运行证据；本票只证明基础 AC 行为，性能策略调优由后续票分别验证。
