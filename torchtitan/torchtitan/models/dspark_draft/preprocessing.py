"""Retained text conversation preprocessing, owned by the draft backend."""
...
class GeneralParser:
    def __init__(self, tokenizer, chat_template):
        ...
        end_of_turn = "(?:" + "|".join(re.escape(value) for value in terminators) + ")"
...
def preprocess_record(
    ...
    assert "conversations" in record, (
        "Expected `conversations` field for JSONL records."
    )
    return parser.parse(
