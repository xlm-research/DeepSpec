...
def _ring_rotate_tensors(tensors: tuple[torch.Tensor, ...], group):
    ...
    previous_global_rank = dist.get_global_rank(group, (group_rank - 1) % group_size)
    received = tuple(
        torch.empty(
            tensor.shape,
            dtype=tensor.dtype,
...
class _RingFlexAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        for step in range(group_size):
            ...
            partial_output, partial_lse = _compiled_context_flex_attention_forward(
                query,
                current_key,
                current_value,
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        def partial_gradients(
            ...
            with torch.enable_grad():
                partial_output, recomputed_lse = compiled_attention(
                    query_for_grad,
                    key_for_grad,
                    value_for_grad,
                    block_mask,
...
class Qwen3DSparkAttention(nn.Module):
    ...
    def forward(
        ...
        context_position_embeddings: Optional[tuple[torch.Tensor, torch.Tensor]] = None,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        v_ctx = (
            ...
            .view(bsz, ctx_len, self.num_key_value_heads, self.head_dim)
            .transpose(1, 2)
        )
        k_noise = self.k_norm(
        ...
        if cp_enabled:
            ...
            if (
                ...
                from .attention import fp32_attention_forward

                attn_fn = fp32_attention_forward
...
class Qwen3DSparkModel(Qwen3PreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.proposal_hidden_offset = int(getattr(config, "proposal_hidden_offset", 0))
        if self.proposal_hidden_offset < 0 or (
            self.proposal_hidden_offset + self.block_size > self.verification_block_size
        ):
            raise ValueError(
    ...
    def forward(
        ...
        if self.context_parallel_size > 1:
            ...
            local_tail_start = (
                2 * self.context_parallel_size - self.context_parallel_rank - 1
            ) * half_context_len
        else:
        ...
        if target_last_hidden_states is not None:
            ...
            if self.context_parallel_size > 1:
                ...
                owns_head = (all_target_pred_indices >= local_head_start) & (
                    all_target_pred_indices < local_head_start + half_context_len
                )
                owns_tail = (all_target_pred_indices >= local_tail_start) & (
                    all_target_pred_indices < local_tail_start + half_context_len
                )
                local_pred_indices = torch.where(
                    owns_head,
                    all_target_pred_indices - local_head_start,
                    half_context_len + all_target_pred_indices - local_tail_start,
