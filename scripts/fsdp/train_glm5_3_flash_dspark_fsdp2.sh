...
draft_ep_backend="${DRAFT_EP_BACKEND:-native}"
draft_ep_max_tokens="${DRAFT_EP_MAX_TOKENS:-4096}"
case "${draft_ep_backend}" in
    native|deepep|auto) ;;
    *) echo "DRAFT_EP_BACKEND must be native, deepep, or auto." >&2; exit 2 ;;
esac
if [[ "${draft_ep_backend}" != "native" && "${target_backend}" == "vllm" \
    && -z "${VLLM_PYTHON_BIN:-}" ]]; then
    echo "Set VLLM_PYTHON_BIN to the existing target interpreter when selecting a draft DeepEP environment; otherwise it inherits PYTHON_BIN." >&2
    exit 2
fi
if [[ ! "${draft_ep_max_tokens}" =~ ^[1-9][0-9]*$ ]]; then
    echo "DRAFT_EP_MAX_TOKENS must be positive." >&2
    exit 2
fi
...
if [[ "${draft_ep_backend}" == "deepep" ]] && ((draft_ep == 1)); then
    echo "DRAFT_EP_BACKEND=deepep requires DRAFT_EP greater than one." >&2
    exit 2
fi
if [[ "${draft_ep_backend}" == "deepep" && "${dry_run}" != "true" ]]; then
    # Fail before staging target weights or producing a teacher partition.
    "${python_bin}" -c 'from deepspec.distributed.deepep_dispatch import require_deepep; require_deepep()'
fi
...
echo "  draft EP backend=${draft_ep_backend}, dispatch token capacity=${draft_ep_max_tokens}"
...
"${launcher[@]}" \
    ...
    --opts "train.parallel.expert_dispatch_backend=${draft_ep_backend}" \
    --opts "train.parallel.expert_dispatch_max_tokens_per_rank=${draft_ep_max_tokens}" \
