...
archive = report.parent / 'deepspec_vllm_torchtitan_envs.tar.gz'
expected = json.loads((report / 'source-manifest.json').read_text())
run_dir = Path(tempfile.mkdtemp(prefix='deepspec-tar-benchmark-', dir='/tmp'))
...
extract_command = ['tar', '-xzpf', str(local_archive), '-C', str(extract_root)]
