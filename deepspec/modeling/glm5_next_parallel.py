...
def parallelize_glm5_next_model(model, *, topology, draft: bool = False):
    ...
    # The target keeps the existing native adapter. Only the draft may opt in
    # to DeepEP, with one buffer shared by its sequential MoE layers.
    expert_dispatcher = None
    if draft:
        from deepspec.distributed.draft_expert_dispatch import build_draft_expert_dispatcher

        expert_dispatcher = build_draft_expert_dispatcher(model, topology=topology)
    for layer in model.layers:
        _parallelize_attention(layer.self_attn, topology=topology)
        if hasattr(layer.mlp, "experts"):
            _parallelize_moe(
                layer.mlp, topology=topology, expert_dispatcher=expert_dispatcher
            )
        else:
