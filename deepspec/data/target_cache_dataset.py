...
def validate_train_cache(
    *,
    ...
    if getattr(draft_model.config, "deepspec_target_execution", None) == (
        "full_model"
    ):
        expected_final_layer = int(
            draft_model.config.deepspec_target_final_hidden_layer
        )
        assert manifest.get("target_execution") == "full_model", (
            "Target cache was generated by a truncated teacher; regenerate it "
            "with the full GLM-5.3 target."
        )
        assert int(manifest.get("target_execution_num_hidden_layers", -1)) == (
            expected_final_layer + 1
        ), "Target cache full-model depth does not match the draft contract."
        assert int(manifest.get("target_final_hidden_layer", -1)) == (
            expected_final_layer
        ), "Target cache final hidden layer does not match the draft contract."
        assert manifest.get("target_final_hidden_source") == (
            "full_model_final_norm"
        ), "Target cache does not contain the normalized full-model final state."
